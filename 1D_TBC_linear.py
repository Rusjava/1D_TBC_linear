...
#  Dielectric constants with the bent term
    alp[MMIN2 + 1:MMAX + 2] = alp0+(np.r_[MMIN2+1:MMAX+2]*h-RMAX)*aa*h**2
    alp[MMIN:MMIN2 + 1] = alp1+(np.r_[MMIN:MMIN2+1]*h-RMAX)*aa*h**2
    alp[0:MMIN] = alp0+(np.r_[0:MMIN]*h-RMAX)*aa*h**2
